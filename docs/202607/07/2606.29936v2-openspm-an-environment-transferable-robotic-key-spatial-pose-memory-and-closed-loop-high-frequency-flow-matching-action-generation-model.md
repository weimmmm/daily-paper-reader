...
tldr: "开放式桌面机器人操作要求系统同时具备语义理解、精确位姿估计和高频动作生成，现有VLA模型缺乏几何约束且训练成本高。OpenSPM构建了空间持久记忆框架，通过语义3D感知和卡尔曼滤波跟踪连续6D位姿，从人类演示中提取关键空间位姿作为可迁移记忆。推理时根据指令检索记忆并通过SE(3)变换迁移到新场景，使用轻量条件流匹配模型生成高频动作块，结合实时反馈和终端残差校正抑制误差累积。在LIBERO-GOAL的10个任务上达到85.6%成功率和1033.3Hz等效控制频率，且AI算力需求极低。结构化持久记忆与闭环校正为可靠高频操作提供了关键支撑。"
source: arxiv
...
motivation: 现有端到端VLA模型缺乏几何约束且训练成本高，难以同时满足开放式操作的语义泛化、精确位姿与高频控制需求。
method: 提出OpenSPM框架，含语义3D感知与卡尔曼滤波跟踪、人类演示关键位姿提取与SE(3)迁移、轻量流匹配动作生成及闭环残差校正。
result: "在LIBERO-GOAL的10个任务上成功率达85.6%，等效控制频率1033.3Hz，且推理算力需求极低。"
conclusion: 结构化空间持久记忆与闭环残差校正对实现可靠、高频的机器人操作至关重要。
---

## 摘要
开放环境下的桌面机器人操作需要系统具备语义理解、精确几何位姿估计和高频动作生成能力。虽然端到端视觉-语言-动作（VLA）模型在语义泛化方面表现出色，但它们通常缺乏细粒度任务所需的显式几何约束，且训练成本高昂。为了弥合高层语义与底层物理执行之间的鸿沟，我们提出OpenSPM，一种开放环境空间持久记忆框架，由空间位姿记忆和流匹配动作生成模型组成。OpenSPM首先利用语义条件化的3D感知和卡尔曼滤波跟踪连续的6D位姿。然后，它从人类演示中提取关键空间位姿，并将其保存为可迁移的、以物体为中心的空间持久记忆条目。在推理过程中，OpenSPM根据自然语言指令检索相关的记忆条目，通过SE(3)变换将空间位姿迁移到新场景，并通过轻量级条件流匹配模型生成高频动作块。结合实时本体感觉状态反馈和终端残差校正，该系统有效抑制了轨迹误差累积。在十个LIBERO-GOAL任务上的评估表明，OpenSPM实现了85.6%的成功率和1033.3 Hz的等效控制频率，同时只需极少的推理AI算力。大量消融实验表明，结构化的空间持久记忆和闭环残差校正在可靠、高频的机器人操作中发挥着关键作用。
