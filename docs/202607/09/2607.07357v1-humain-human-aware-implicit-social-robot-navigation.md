...
tldr: "社交机器人导航需感知人类行为，常通过骨架线索。HumAIN框架通过知识蒸馏将隐式社交线索融入规划循环。教师模型融合多模态输入学习鲁棒表征，学生模型轻量化实时推理。在多项指标上平均提升29.8%，实现高效类人导航意识。"
source: arxiv
...
motivation: 现有方法忽略隐式社交线索，预测-规划存在鸿沟，且难以在资源受限平台部署。
method: 用Transformer教师模型融合历史图像、骨架关键点、机器人状态和目标；通过知识蒸馏训练轻量学生模型，优化轨迹重建和特征对齐。
result: "相比最优基线，轨迹预测指标平均提升29.8%。"
conclusion: 利用全身隐式线索，在资源受限平台上实现鲁棒、适应性强的类人导航意识。
---

## 摘要
有效的社交机器人导航需要对人类行为的敏感性，这种敏感性通常通过微妙的骨骼线索（如步态和方向）表现出来。我们提出了人类感知的隐式社交机器人导航（HumAIN），这是一种新颖的框架，通过知识蒸馏将隐式社交线索直接融合到规划循环中。我们首先采用基于Transformer的教师模型，融合丰富的多模态输入，包括历史图像、骨骼关键点、机器人状态和机器人的目标，学习鲁棒的人类感知表征以用于机器人的未来轨迹规划。为了实现实时部署，我们随后将这些知识蒸馏到一个轻量级的学生模型中。通过同时优化轨迹重建和与教师的潜在特征对齐，学生模型学会从最小输入中推断复杂的社交动态。通过有效的蒸馏架构弥合预测与规划之间的差距，我们的方法使机器人能够以自适应、鲁棒且符合社交规范的方式推理人类行为。我们通过大量实验验证了HumAIN，在所有指标上，与最先进的基线相比，轨迹预测指标平均提高了29.8%。这些结果突显了在资源受限平台上利用隐式全身线索实现类人导航意识的好处。
